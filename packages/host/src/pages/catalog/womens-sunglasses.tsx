...
const Products = (props: { facets: any }) => {
  ...
  const backgroundImage = isMobile
    ? `url(${ImageUrlConstants.CATALOG_PAGE_BANNER.WOMEN_SUNGLASSES_BANNER.DESKTOP})`
    : `url(${ImageUrlConstants.CATALOG_PAGE_BANNER.WOMEN_SUNGLASSES_BANNER.MOBILE})`;
  const brand = useGetBrand();
  return (
    <>
      <Head>
        <title>{`Women's Sunglasses Online & In-store Near You - ${BRAND_NAME[brand]}`}</title>
        <meta
          name="description"
          content="Protect your vision and stay in fashion with the latest women’s sunglasses. Discover our wide range of prescription sunglasses near you in-store & online."
        />
        ...
        >
          <div className="banner_text_section">
            <div className="banner_title">
              ...
              <p>A fresh batch of fun shades for every sunny event</p>
            </div>
          </div>
        </div>
...
//   const query = facetKeyMappingWithPathName.CATALOG_WOMEN_SUNGLASSES;
...
export async function getServerSideProps(context: any) {
  ...
  let query = facetKeyMappingWithPathName.CATALOG_WOMEN_SUNGLASSES;
